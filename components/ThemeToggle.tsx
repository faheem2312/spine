...
export default function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="relative w-16 h-9 bg-stone-950 border border-stone-800 rounded-full flex items-center justify-between px-2 cursor-pointer focus:outline-none focus:ring-2 focus:ring-stone-500 hover:border-stone-600 transition-all duration-200"
      aria-label="Toggle theme"
    >
      {/* Sliding background knob */}
      <span
        className={`absolute top-[4px] left-[4px] w-7 h-7 rounded-full bg-stone-50 shadow-md transition-all duration-300 ease-out ${
      ...
      <svg
        ...
        strokeWidth={1.5}
        stroke="currentColor"
        className={`w-4.5 h-4.5 z-10 transition-colors duration-200 ${
